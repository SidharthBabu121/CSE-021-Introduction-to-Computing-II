...
public class ShopArr {
	...
	private void intro(Scanner input) {
        ...
        System.out.println("We sell " + x + " kinds of Cheese (in 0.5 lb packages)");
        ...
        if (x > 0) {
    		...
    		for(int i = 0; i < x;i++) {
    			...
    			while (((cheese[i].getAmount()) < 0 )||((cheese[i].getAmount()) % 0.5 != 0 ) ) {
	...
	private double[] discountSpecials() {
		...
		if (hfAmt > 0) {
			disSpecials[0] = ((int)cheese[0].getAmount()*(cheese[0].getPrice()/2));
		}

		if(rhAmt > 0) {
			disSpecials[1] = (int)(cheese[1].getAmount()/1.5)*cheese[1].getPrice()/2;	
		}
	...
	private double printSubTotals(double subTotal, double[] disSpecials) {
		...
		else if ((disSpecials[0] + disSpecials[1]) == 0){
			System.out.println("None                             -$0.0");
		} 
		
		double total = subTotal - (disSpecials[0] + disSpecials[1]);
	...
	private void printFinalTotal(double newSubTotal) {
		...
		System.out.println("Additional " + (int)disc  +"% Discount:          -$" +  ((newSubTotal)*(disc/100)) );
		double total = (newSubTotal) - ((newSubTotal)*(disc/100));
